...
```
```{r}
reg1 <- feols(df, unemployment_rate~treated + retail + treated*retail)
etable(reg1)
```
...
reg2 <- feols(df2, unemployment_rate~treated + retail + treated*retail)
etable(reg2)
```
...
reg3 <- feols(df3, unemployment_rate~retail + treated + treated*retail)
etable(reg3)
```
